...
const List = () => {
  ...
  useEffect(() => {
    //use for fetch birthday when page render
    fetchPeople();
  }, []);

  const fetchPeople = async () => {
    // fetch birthdays list from database
    await axios
  ...
  const deletePeople = (id) => {
    const delId = id;
    axios.delete(`http://localhost:3001/delete/${delId}`).then((response) => {
      setPeople(
        people.filter((val) => {
          return val.id !== id;
        })
  ...
  const clearAll = async () => {
    // clear all bday from list
    await axios
      .delete("http://localhost:3001/clear")
      .then((response) => {
        setPeople([]);
      })
      .catch((err) => {
    ...
    <>
      <section className="container">
        ...
        {people.map((person) => {
          ...
          const birthdayCheck = today - bday.getDate() === 0
          return (
